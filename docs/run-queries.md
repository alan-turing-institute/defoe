...
spark-submit --py-files lwm.zip lwm/<MODEL>/query_runner.py [<NUM_CORES>]
```
...
```bash
cd job-book
spark-submit --py-files lwm.zip lwm/books/query_runner.py
```
...
```bash
cd job-papers
spark-submit --py-files lwm.zip lwm/papers/query_runner.py
```
...
```bash
nohup spark-submit --py-files lwm.zip lwm/books/query_runner.py 144 > log.txt &
```

```bash
nohup spark-submit --py-files lwm.zip lwm/papers/query_runner.py 144 > log.txt &
```
...
```bash
cd <JOB_BUNDLE_DIRECTORY>
pyspark < lwm/<MODEL>/query_runner.py [<NUM_CORES>]
```

For example:

```bash
pyspark < lwm/books/query_runner.py
```

```bash
pyspark < lwm/papers/query_runner.py
```
